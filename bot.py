...
def whatsapp_webhook():
    ...
    try:
        # --- PRUEBA: si el mensaje es "Hola", respondemos con un texto simple ---
        if incoming_msg.lower() == "hola":
            resp.message("Hola, esto es una prueba. Si ves esto, el bot responde correctamente.")
            logger.info("📨 Mensaje de prueba enviado.")
            session["estado"] = "START"  # Reiniciamos para que vuelva a mostrar el menú la próxima vez
            return str(resp)
        ...
        if session["estado"] == "REVIEW_DATA":
            if incoming_msg.lower() in ["si", "sí", "yes"]:
                try:
                    ...
                    # --- ENVIAR SOLO A LA DUEÑA (NO AL CLIENTE) ---
                    if OWNER_WHATSAPP:
                        mensaje_duena = (
                            ...
                            f"🔗 Descarga: {BASE_URL}/descargar/{output_filename}"
                        )
                        enviar_mensaje_whatsapp(OWNER_WHATSAPP, mensaje_duena)

                    # --- MENSAJE PARA EL CLIENTE (SOLO TEXTO) ---
                    resp.message(
                        "✅ ¡Solicitud enviada correctamente!\n\n"
                        "📌 La encargada de la Papelería Líder revisará tu documento y te contactará en breve.\n"
